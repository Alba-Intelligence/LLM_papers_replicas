...
The next milestone is to bring the same full-model-training honesty to DeepSeek without prematurely forcing all model internals into Lux layers.
...
1. extend full-model training beyond the current OpenMythos bootstrap surface into DeepSeek full-model paths,
